...
""" % (sys.argv[0], datetime.utcnow().isoformat()), file=f)


def introduce(f, testname):
    print("""#[test]
fn test_%s() {""" % testname, file=f)
...
def get_ca(f):
    if not os.path.exists('data/fake-ca.der'):
        subprocess.check_call(['openssl',
            ...
            '-inform', 'PEM',
            '-noout',
            '-in', 'x509test/ca/fake-ca.cert',
            '-out', 'data/fake-ca.der'])
    print("""    let ca = include_bytes!("data/fake-ca.der");
    let ca = webpki::TrustAnchor::try_from_cert_der(&ca[..])
        .expect("cannot parse ca cert");""", file=f)
...
def valid_chain(f, _chain):
    ...
    print("""    let ee = webpki::EndEntityCert::try_from(&include_bytes!("%s")[..])
    ...
    ee.verify_is_valid_tls_server_cert(&ALL_SIG_ALGS,
        ...
        .expect("ee cert should be valid");""" % (leaf, int(time.time())), file=f)
...
def invalid_chain(f, _chain, error):
    ...
    print("""    assert_eq!(
        ...
        webpki::Error::%s
    );""" % (leaf, int(time.time()), error), file=f)
...
def emit_anchor_case(f, case, cert):
    ...
    introduce(f, n)
    print("""    let ca = include_bytes!("%s");
    assert_eq!(
        webpki::TrustAnchor::try_from_cert_der(&ca[..])""" % cert, file=f)

    if error:
        print("""        .expect_err("anchor parsing error expected"),
        webpki::Error::%s
    );""" % error, file=f)
    else:
        print("""        .is_ok(), true
